...
case class LineitemOperatorManager() extends AbstractTableOperatorManager {
  ...
  val typeSchema = "(f => (f(0).toInt, f(1).toInt, f(2).toInt, f(3).toInt, f(4).toFloat, f(5).toFloat, f(6).toFloat, f(7).toFloat, f(8), f(9), java.sql.Date.valueOf(f(10)), java.sql.Date.valueOf(f(11)), java.sql.Date.valueOf(f(12)), f(13), f(14), f(15)))"
  ...
  override val sortFields: Map[String, String] = Map(
    ...
    "LINESTATUS" -> "10",
    "SHIPDATE" -> "11.getTime",
    "COMMITDATE" -> "12.getTime",
    "RECEIPTDATE" -> "13.getTime",
    "SHIPINSTRUCT" -> "14",
    "SHIPMODE" -> "15",
    "COMMENT" -> "16"
  )
