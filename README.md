...
## Modularized DataFarm
### Getting input Spark
For the input, you need Spark Jobs using either RDD API or DataFrame API.
- From RDD Job, we need to get RDD's lineage graph (example in `/SparkToUnifiedJson/debugString`).
  To generate it run `toDebugString()` on your resulting RDD (in the jobs source code)
- From DataFrame Job, we need to get the logical plan. Use `df.queryExecution.optimizedPlan.toString` where `df`
  is your Job's resulting DataFrame.

Having debugString or plans, you need to execute one of the following notebooks: `/SparkToUnifiedJson/debugStringToJson.ipynb` or
`/SparkToUnifiedJson/logicalPlanToJson.ipynb`.
This will generate Unified Jsons.
### Getting Input SQL
Run `/SqlToUnifiedJson/SqlToJson.ipynb` on desired sql queries (in .txt format). Be sure that PostgreSQL Server
configuration inside the notebook is correct.

### Generate and Forecast Jobs
#### To generate Spark Jobs
1. put unified jsons to `/data-farm/data/input_workload_exec_plan`
   and set `UNIFIED_JSON = True` in config (please keep it on `False` when using the old implementation).
2. Set `JOB_TYPE` to `SPARK`
3. Run `RunGenerator.py`
#### To generate SQL Jobs
1. Execute `DataFarmSqlJobGenerator.ipynb` to generate jobs
2. Put them to `data/Experiment_folder/generated_jobs`
#### To forecast Labels

1. Set `EXPERIMENT_JOB` in config to desired job type to run the forecasting process on.
2. Don't forget to fill Spark(or Postgres DB) config accordingly to your system.
3. Also, for `SQL`, set `FEATURE_COLS` to variant without complexities.
4. Run `RunLabelForecaster.py`

### Input Dataset
For the input dataset, I have used tpch-data generated by dbgen included in https://github.com/ssavvides/tpch-spark
You can generate different sizes TPCHs Datasets.
Don't forget to set `GENERATED_JOB_INPUT_DATA_PATH` to your data.
