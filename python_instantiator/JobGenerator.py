        ...
        for op in branch_operators:
            ...
            if operator.op_pact == "Join":
                ...
                if idx + 1 < len(branch_operators) and branch_operators[idx+1].split("_")[0] == "Group by":
                    skip_next = True
                    next_op = AbstractOperator.AbstractOperator(branch_operators[idx+1], job_info=job_info) # table_operator_manager seems to be needed, but is not inserted yet
        ...
        for name in files:
            ...
            for i in range(params["nVersions"]):
                ...
                if job_seed < 0:
                    job_seed = job_nr
                try:
                    job_generator = JobGenerator(abstract_plan, tpch_dataset_op_mng, job_info, seed = 10*job_seed + i + job_nr)
                #generated_job = job_generator.generate(abstract_plan.node["Data Source_0"]., job_id, saveExecutionPlan=True)
                    generated_job = job_generator.generate("Data Source_0", job_id, save_execution_plan=True, job_info=job_info)
                # Again something is done with the JobInfoRecorder
                    #print(str(job_info))
