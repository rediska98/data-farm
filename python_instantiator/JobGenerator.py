...
from python_instantiator.JobInfoRecorder import JobInfoRecorder as JobInfoRecorder
...
from python_instantiator import JsonUtil

MAX_INT = 100000  # Random max number for the random init


class JobGenerator:
    def __init__(self, abstract_plan, dataset_operator_manager, job_info=None, seed=0):
        ...
        self.table_sequence, self.join_sequence = self.dataset_operator_manager.get_source_table_sequence(self.n_joins,
                                                                                                          self.seed)

        # Again something is done with the JobInfoRecorder
        if job_info:
            job_info.table_sequence = self.table_sequence
            job_info.join_relations_sequence = self.join_sequence
    ...
    def get_op_code(self, op, next_op=None, second_op=None) -> str:
        ...
        # I have changed map with reduce since map is very rare in abstract plan generated from sql
        # unified json. Aggregate(Reduce) appears much more often.
        gen_code_dict = {
            "Data Source": self.handle_data_source,
            "Map": self.handle_reduce,
            "Filter": self.handle_filter,
            "Reduce": self.handle_map,
    ...
    def handle_group_by(self, op, next_op):
        has_reduce = ("Reduce" in self.operators)
        group_by, field = op.group_by_code(self._prev_vars.pop(-1), next_op, random.randint(0, MAX_INT),
                                           self.used_groups, has_reduce)
        if field:
    ...
    def handle_sort_partition(self, op, next_op):
        has_group = ("Group by" in self.operators) and ("Reduce" in self.operators)
        return op.sort_partition_code(self._prev_vars.pop(-1), has_group, random.randint(0, MAX_INT),
    ...
    def _generate(self, entry_op_id, abstract_plan=None, job_info=None):
        ...
        for op in branch_operators:
            operator = AbstractOperator.AbstractOperator(op, table_operator_manager=table_op_manager, job_info=job_info)
            op_code = ""
            ...
            if operator.op_pact == "Join":
                ...
                before_join_code = self._generate(source, new_abstract_plan, job_info=job_info)
                ...
                #idx = operator.n_id
                idx = branch_operators.index(operator.op_id)

                if idx + 1 < len(branch_operators) and branch_operators[idx + 1].split("_")[0] == "Group by":
                    skip_next = True
                    next_op = AbstractOperator.AbstractOperator(branch_operators[idx + 1],
                                                                job_info=job_info)  # table_operator_manager seems to be needed, but is not inserted yet
                    op_code = self.get_op_code(operator, next_op)
                else:
                    op_code = self.get_op_code(operator)

            elif operator.op_pact == "Map":  # was Reduce here
                prev_op_id = operator.op_id
                continue
    ...
    def generate(self, entry_op_id, job_name, save_execution_plan=False, job_info=None):
        self.operators = self.abstract_plan.get_operators()
        operator_codes = self._generate(entry_op_id, job_info=job_info)
...
def parse_args(args):
    ...
    if args.__len__() >= 5:
        ...
        raise Exception(
            "Expected arguments: <nJobs> <nVersions> <dataManager> <abstractPlansSource> <genJobsDest> [jobSeed]")
    return params
...
def main(args):
    ...
    for root, dirs, files in os.walk(params["abstractPlansSource"], topdown=False):
        for name in files:
            ...
            for i in range(params["nVersions"]):
                ...
                job_info = JobInfoRecorder(job_id)
                ...
                if job_seed < 0:
                    job_seed = job_nr
                try:
                    job_generator = JobGenerator(abstract_plan, tpch_dataset_op_mng, job_info,
                                                 seed=10 * job_seed + i + job_nr)
    ...
    JsonUtil.persist(JsonUtil.to_json(jobs_info), params["genJobsDest"])
